/*
Template for Oracle Deploy Script for Non Prod feeder jobs.
parameters needs to be replaced as per Project:
deploy_params section is mostly one time setup based on project environments (TEST)
...
def deploy_params = [
          ...
          TEST: [
			database_source: 'stletl3_dev',
            database_target: 'stletl3_tst',
            generic_job: '../NonProd/GenericOracleDEPLOY',
...
pipeline {
    ...
    parameters {
		choice(name: 'deploy_environment', choices: ['TEST'], description: 'Choose the Deployment Environment')
    ...
    stages {
        stage("Execute Oracle Deploy Script") {
            steps {
                script {
